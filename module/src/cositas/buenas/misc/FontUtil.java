...
//        Arrays.stream(Toolkit.getDefaultToolkit().getFontList()).forEach(s -> System.out.println(s));
        ...
        for (Font font : GraphicsEnvironment.getLocalGraphicsEnvironment().getAllFonts()) {
            System.out.println(i.incrementAndGet() + ". [" + getUniqueFontName(font) + "] / " + font.getName() + "/" + font.getFamily() + "/" + font.getFontName());
        }
    }
    ...
    public static String getUniqueFontName(Font font) {
        ...
        if (suffix.length() > 0 && font.getFamily().endsWith(suffix)) suffix = "";
        return font.getFamily() + suffix;
    }
}
